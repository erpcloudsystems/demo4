...
class CustomWorkflowAction(Document):
...
def check_system_admin(user):
    # Query the database to get the designation of the employee with the given user_id
    system_admin = frappe.db.sql("""
        SELECT designation FROM `tabEmployee` e
        WHERE user_id = %s
    """, (user), as_dict=True)
    
    # Check if the designation of the first (and only) record is 'System Admin'
    if system_admin and system_admin[0]['designation'] == 'System Admin':
        return True

    # Return False if the user is not a System Admin
    return False

def check_if_alowed_action(doc, user, doctype):
    # Get the custom branch from the document
    current_branch = doc.get('custom_branch2')
    # Get the current workflow state of the document
    current_state = get_doc_workflow_state(doc)
    
	# Proceed if there is a custom branch specified in the document
    if current_branch:
        # Get the initial state of the workflow for 'Sales Order' documents
        initial_state = frappe.db.sql("""
            SELECT wds.state 
            FROM `tabWorkflow` w
            JOIN `tabWorkflow Document State` wds ON wds.parent = w.name
            WHERE w.document_type = %s
            LIMIT 1
            """, (doctype), as_dict=True)[0].get('state')
        ...
        # If the branch is not found and the document is not in its initial state, return False
        if not found and current_state != initial_state:
            return False

    # If the custom branch is not specified or if the conditions are met, return True
    return True
...
def process_workflow_actions(doc, state):
	...
	# Restrict action if user is not a system admin or lacks branch permissions
	if not check_system_admin(current_user) and not check_if_alowed_action(doc, current_user, doc.get('doctype')):
		frappe.throw("This user can't approve this action duo to branch restriction.")
